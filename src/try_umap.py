...
def nearest_neighbors(
    ...
    verbose: bool (optional, default False)
        Whether to logger status data during the computation.
    ...
    """
    if verbose:
        logger( "Finding Nearest Neighbors")
    ...
    if verbose:
        logger( "Finished Nearest Neighbor Search")
...
def simplicial_set_embedding(
    ...
    if densmap or output_dens:
        if verbose:
            logger(" Computing original densities")
    ...
    if output_dens:
        if verbose:
            logger(" Computing embedding densities")
...
class UMAP(BaseEstimator):
    ...
    def __init__(
        ...
        logger('UMAP __init__')
        use_caching = use_caching and knn_cache_file is not None
        ...
        self.knn_cache_file = knn_cache_file
        self.SUBSAMPLE_DATA = SUBSAMPLE_DATA # flag related to file i/o and logging
    ...
    def fit(self, X, y=None, force_all_finite=True, **kwargs):
        ...
        **kwargs : optional
        ...
        if self.verbose:
            logger( "Construct fuzzy simplicial set")
        ...
        if self.metric == "precomputed" and self._sparse_data:
            ...
            # logger("Computing KNNs for sparse precomputed distances...")
        ...
        # Handle small cases efficiently by computing all distances
            ...
            if self.knn_dists is None:
                if self.use_caching and pickle_exists(self.knn_cache_file):
                    tmp = read_pickle(self.knn_cache_file)
                    self._knn_indices = tmp["self._knn_indices"]
                    self._knn_dists = tmp["self._knn_dists"]
                    self._knn_search_index = tmp["self._knn_search_index"]
                    del tmp
                else:
                    (
                        self._knn_indices,
                    ...
                    if self.use_caching:
                        save_pickle(self.knn_cache_file, {
                            "self._knn_indices": self._knn_indices,
                            "self._knn_dists": self._knn_dists,
                            "self._knn_search_index": self._knn_search_index
                            })
            else:
                self._knn_indices = self.knn_indices
                self._knn_dists = self.knn_dists
